...
function FormElementOptions({
  ...
  return (
    <Form {...form}>
      <form onSubmit={handleSubmit(onSubmit)} className="pt-3 border-t border-dashed">
        <div>
                  ...
                  <div className="flex flex-col gap-2 border rounded-lg w-fit p-3 ">
                    ...
                    {options.map((opt, i) => (
                      <div key={opt.value} className="flex gap-2 items-center">
                        ...
                        <Button onClick={() => removeOption(i)} type="button" className="duration-100" variant={"ghost"}>
                          {" "}
                          <X size={16} />
                        </Button>
                      </div>
...
export function FieldCustomizationView({
  ...
  const SavedFormElementOptions = () => <FormElementOptions fieldIndex={fieldIndex} stepIndex={stepIndex} j={j} {...formElement} close={close} />;
  if (isDesktop) {
    return (
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          <Button type="button" variant="ghost" size="icon" className=" h-9 hover:bg-gray-200">
            <FaEdit />
          </Button>
        </DialogTrigger>
  ...
  return (
    <Drawer open={open} onOpenChange={setOpen}>
      <DrawerTrigger asChild>
        <Button type="button" variant="ghost" size="icon" className="rounded-xl h-9">
          <FaEdit />
        </Button>
      </DrawerTrigger>
      <DrawerContent>
